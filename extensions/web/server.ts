...
function handleLoginPost(req: http.IncomingMessage, res: http.ServerResponse): void {
	...
	req.on("end", () => {
		...
		// pi-lab fork: only same-site path redirects — "/foo" yes, "//evil.com"
		// and absolute URLs no (open-redirect hardening).
...
export function start(port: number = 4100): string {
	...
	server = http.createServer(async (req, res) => {
		...
		// pi-lab fork: same-origin only — upstream sent Access-Control-Allow-Origin: *
		// on every response, which invites cross-origin probing of an authenticated
		// control surface. The PWA and all mounts are same-origin (also behind the
		// hub's path-prefix proxy), so no CORS headers are needed. If cross-host
		// connect from the PWA is ever wanted again, allowlist origins explicitly.
		...
		if (!isApiPath) {
			// pi-lab fork: a valid full Bearer token authenticates pages too — the
			// pi-hub proxy injects it, so hub-authed users reach /mobile etc.
			// without a second per-session cookie login.
			const header = req.headers.authorization;
			const bearer = header?.startsWith("Bearer ") ? header.slice(7) : null;
			const bearerOk = apiToken !== null && tokensEqual(bearer, apiToken);
			if (!bearerOk) {
				if (authCredentials) {
	...
	// pi-lab fork: bind loopback only (Tailscale Serve is the sole external
	// surface — never expose raw HTTP on tailnet/LAN interfaces), and handle
	// listen errors instead of crashing the whole pi process on EADDRINUSE
	// when a second session races for the same port.
	server.on("error", (err: NodeJS.ErrnoException) => {
		log("listen-error", { port, code: err.code, error: err.message }, "ERROR");
		server = null;
		serverPort = null;
	});
