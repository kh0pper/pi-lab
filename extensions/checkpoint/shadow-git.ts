...
export class ShadowGit {
	...
	private run(args: string[], timeoutMs?: number): Promise<{ stdout: string; stderr: string; code: number }> {
		return new Promise((resolvePromise) => {
			execFile(
				...
				{ cwd: this.workTree, timeout: timeoutMs ?? this.timeoutMs, maxBuffer: 8 * 1024 * 1024 },
				(err, stdout, stderr) => {
					const code = err ? ((err as { code?: number }).code as number | undefined) ?? -1 : 0;
					resolvePromise({ stdout: stdout ?? "", stderr: stderr ?? "", code: typeof code === "number" ? code : -1 });
				},
			);
		});
	}
